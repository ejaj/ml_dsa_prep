...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.blocks = nn.Sequential(
         ...
         # --- Final norm then vocab projection ---
        self.final_norm = nn.LayerNorm(model_dim)
        self.vocab_head = nn.Linear(model_dim, vocab_size)
         
        # Optional sanity check (matches your constraint)
        assert model_dim % num_heads == 0, "model_dim must be divisible by num_heads"

        # Save for forward usage
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        B, T = context.shape
        # token embeddings
        tok = self.word_emb(context) # (B, T, D)

        # position embeddings (0..T-1)
        pos_idx = torch.arange(T, device=context.device)  # (T,)
        pos = self.pos_emb(pos_idx)                       # (T, D)
        x = tok + pos                                     # broadcast to (B, T, D)
        ...
        # vocab projection + softmax over vocab
        logits = self.vocab_head(x)                       # (B, T, V)
